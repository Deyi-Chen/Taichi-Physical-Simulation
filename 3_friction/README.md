...
$$
v = \begin{bmatrix} v_p \\ v_s \end{bmatrix} \in \mathbb{R}^6
$$
...
$$
P_f(x) = \sum_k \mu \lambda_k^n \, f_0\!\left( \|\bar{v}_k\| \, \hat{h} \right)
$$

where:

$$
\bar{v}_k = (T_k^n)^T v(x)
$$
...
To avoid non-differentiability at $\|v\| = 0$, we define a smooth function $f_0(y)$ as:
...
which acts as a smooth approximation of the Coulomb friction coefficient. We define:
...
$$
\nabla_x P_f(x)
= \sum_k \mu \lambda_k^n \frac{\partial f_0(\|\bar{v}_k\| \hat{h})}{\partial x}
$$
...
\nabla_x P_f(x)
= \sum_k \mu \lambda_k^n
\cdot Df_0(\|\bar{v}_k\| \hat{h})
\cdot \frac{\bar{v}_k}{\|\bar{v}_k\|}
\cdot (T_k^n)^T
\cdot \frac{\partial v}{\partial x}
$$
...
where $H_k$ is the Hessian with respect to the local velocity $\bar{v}_k$:

$$
H_k
= f_1'(\|\bar{v}_k\|) \frac{\bar{v}_k \bar{v}_k^T}{\|\bar{v}_k\|^2}
+ \frac{f_1(\|\bar{v}_k\|)}{\|\bar{v}_k\|}
\left( I - \frac{\bar{v}_k \bar{v}_k^T}{\|\bar{v}_k\|^2} \right)
$$
...
$$
Dg(v)
= f_1'(\|v\|) \frac{v v^T}{\|v\|^2}
+ \frac{f_1(\|v\|)}{\|v\|}
\left( I - \frac{v v^T}{\|v\|^2} \right)
$$
